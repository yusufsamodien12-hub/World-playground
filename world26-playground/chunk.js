...
window.$RefreshSig$ = () => (type) => type;</script>
    ...
    <title>Underworld Simulation v1.0.2</title>
  </head>
  <body>
    <div id="root">
      <div style="position: fixed; inset: 0; display: flex; align-items: center; justify-content: center; background: #050505; color: #fff; font-family: monospace;">
        <div style="text-align: center;">
          <div style="font-size: 24px; font-weight: bold; margin-bottom: 16px;">â¡ Loading Architect-OS...</div>
          <div id="loading-status" style="font-size: 12px; opacity: 0.5;">Initializing Neural Pathways</div>
        </div>
      </div>
    </div>
    <script type="module" src="/src/main.tsx"></script>
    <script>
      // Detect if JavaScript is running
      console.log('â HTML loaded, JavaScript executing');
      setTimeout(() => {
        const status = document.getElementById('loading-status');
        if (status) {
          status.textContent = 'Loading JavaScript modules...';
        }
      }, 100);
      ...
      setTimeout(() => {
        if (document.getElementById('root').children.length === 1) {
          const status = document.getElementById('loading-status');
          if (status) {
            status.innerHTML = '<span style="color: #ff4444;">â Failed to load application<br/><small>Check browser console for errors</small></span>';
          }
          console.error('â React app failed to mount after 10 seconds');
        }
      }, 10000);
    </script>
  </body>
